...
async def push_limited_traffic_usage(subscription: Subscription, user, used_gb: float, limit_gb: int) -> None:
    ...
    не умеет отдать расход конкретно LIMITED-пула. Чтобы клиентское приложение
    могло показать его отдельной строкой в списке серверов, панельный скрипт
    подставляет эти цифры сам — но узнать их может только отсюда. Best-effort:
...
async def process_limited_traffic(
    ...
    await push_limited_traffic_usage(subscription, user, subscription.limited_traffic_used_gb or 0.0, limit_gb)
