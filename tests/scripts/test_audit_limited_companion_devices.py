...
решает, что чинить. Неверная подсказка отправит чинить не то: «упёрлись в
лимит» лечится пересинхронизацией лимита, а «исторический хвост» — не
лечится ничем, само пройдёт на следующем обновлении подписки в приложении
(подтверждено вручную на реальном hwid: отклонённый ранее на заниженном
лимите компаньона, он появился на компаньоне после одного повторного
запроса ссылки подписки — без пересоздания устройства).
"""
...
def test_calls_it_a_stale_gap_when_neither_side_is_currently_full():
    """Лимиты одинаковые и обе стороны свободны сейчас — панель отказать не могла
    СЕГОДНЯ; расхождение объясняется тем, что лимит когда-то был ниже."""
    item = _divergence()

    assert 'исторический хвост' in item.reason()
    assert 'забит под лимит' not in item.reason()


def test_does_not_blame_a_limit_the_panel_never_reported():
    """hwidDeviceLimit=None — безлимит: обвинять его в отказе нельзя, это тоже хвост."""
