...
- `app/services/limited_squad_monitoring_service.py` — Python-модуль
  Классы: `LimitedSquadMonitoringService` (4 методов)
  Функции: нет
...
- `tests/services/test_limited_squad_monitoring_service.py` — Python-модуль
  Классы: нет
  Функции: `test_only_subscriptions_on_limited_enabled_tariffs_are_selected`, `test_expired_subscriptions_are_excluded`, `test_trial_and_limited_status_subscriptions_are_included`, `test_orphaned_query_finds_subscriptions_left_active_after_tariff_disabled`, `test_orphaned_query_ignores_still_enabled_tariffs` — limited_squad_active=True на включённом тарифе — штатный случай, его, `test_orphaned_query_ignores_subscriptions_with_squad_already_off`, `test_notify_exhausted_sends_message_with_topup_button`, `test_notify_exhausted_escapes_html_in_tariff_name` — Имя тарифа — админский ввод; попадает в сообщение с parse_mode='HTML'., `test_notify_exhausted_omits_topup_cta_when_topup_disabled` — TRAFFIC_TOPUP_ENABLED=false — handle_add_traffic_limited отклонит, `test_notify_exhausted_noop_without_bot`, `test_notify_exhausted_noop_without_telegram_id`, `test_notify_exhausted_respects_global_notifications_toggle`, `test_notify_exhausted_respects_user_traffic_warning_pref`, `test_notify_exhausted_swallows_send_errors`, `test_run_cycle_notifies_on_transition_to_inactive`, `test_run_cycle_does_not_notify_when_still_active`
