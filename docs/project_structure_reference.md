...
- `app/handlers/subscription/traffic.py` — Python-модуль
  Классы: нет
  Функции: `handle_add_traffic`, `handle_reset_traffic`, `confirm_reset_traffic`, `refresh_traffic_config`, `get_traffic_packages_info`, `select_traffic`, `add_traffic`, `handle_add_traffic_limited` — Экран выбора пакета докупки трафика для компаньон-аккаунта лимитного сервера., `add_traffic_limited` — Списывает деньги и докупает трафик компаньон-аккаунту лимитного сервера., `handle_no_traffic_packages`, `handle_switch_traffic`, `confirm_switch_traffic`, `execute_switch_traffic`
...
- `app/services/limited_squad_service.py` — Python-модуль
  Классы: нет
  Функции: `is_limited_traffic_enabled` — Включена ли LIMITED-механика (новая архитектура) на тарифе., `get_limited_squad_uuids` — UUID squad'ов LIMITED-пула тарифа. Пусто, если механика выключена., `get_raw_limited_squad_uuids` — То же самое, но без гейта на ``limited_traffic_enabled``., `get_limited_squad_uuids_for_subscription` — То же самое, но по подписке — сама подгружает тариф по ``tariff_id``., `get_limited_base_traffic_gb` — Базовый лимит LIMITED-пула в ГБ, без докупок. 0 = механика выключена., `resolve_main_panel_user_id` — Тот же id, что уходит в panel_sync для MAIN-аккаунта подписки., `get_active_limited_traffic_purchases_gb` — Housekeep + сумма активных докупок LIMITED-пула., `add_limited_traffic_purchase` — Регистрирует докупку LIMITED-пула с истечением через 30 дней., `remove_limited_traffic_purchase` — Списывает докупленный LIMITED-пул, не трогая базу тарифа., `get_effective_limited_traffic_limit_gb` — База тарифа + активные докупки. 0 = безлимит (либо механика выключена)., `resolve_limited_node_uuids` — Ноды всех LIMITED squad'ов тарифа, одним списком без дублей., `fetch_limited_used_bytes` — Потребление конкретного юзера по конкретным LIMITED-нодам за период., `refresh_limited_traffic_usage` — Пересчитывает и сохраняет ``subscription.limited_traffic_used_gb``., `push_limited_traffic_usage` — Публикует usage LIMITED-пула на сторону панели (``subscription-merger``)., `sync_limited_squad_state` — Снимает/возвращает LIMITED squad на ``activeInternalSquads`` ОСНОВНОГО юзера., `process_limited_traffic` — Один цикл для подписки: обновить usage, затем при необходимости, `deactivate_orphaned_limited_squad` — Снимает LIMITED squad(ы) тарифа с ОСНОВНОГО юзера подписки, чей тариф
...
- `migrations/alembic/versions/0118_tariff_is_highlighted.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0119_limited_companion_columns.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0120_limited_companion_traffic_topup.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `migrations/alembic/versions/0130_user_panel_identity_backfill.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0131_withdrawal_request_last_reminder_at.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
- `migrations/alembic/versions/0132_user_reminders.py` — Python-модуль
  Классы: нет
  Функции: `upgrade`, `downgrade`
...
- `tests/cabinet/test_admin_limited_companion_view.py` — Python-модуль
  Классы: нет
  Функции: `companion_enabled`, `test_builder_exposes_companion_usage_not_just_its_limit`, `test_builder_adds_active_top_ups_to_the_companion_limit`, `test_builder_leaves_companion_fields_at_defaults_without_a_companion`, `test_sync_action_resyncs_and_returns_fresh_companion_state`, `test_sync_action_surfaces_a_failed_panel_sync`, `test_sync_action_rejects_a_subscription_without_a_companion`
...
- `tests/database/crud/test_limited_companion_base_traffic.py` — Python-модуль
  Классы: нет
  Функции: `test_trial_subscription_uses_own_traffic_limit`, `test_trial_subscription_with_unlimited_traffic_stays_unlimited`, `test_paid_subscription_uses_fixed_settings_value`, `test_total_limit_adds_purchased_to_fixed_base`, `test_paid_subscription_uses_tariff_override_for_companion_squad` — Тариф может задать свой лимит компаньона через ``server_traffic_limits``, `test_tariff_override_falls_back_to_settings_when_not_set_for_squad`, `test_tariff_override_of_zero_falls_back_to_settings` — ``0`` в редакторе лимитов означает «по общему значению», не «безлимит»., `test_tariff_override_ignored_when_companion_squad_uuid_not_configured`, `test_traffic_gb_for_tariff_returns_override_directly` — Прямой вход для показа "сколько дадут" ДО покупки — на экране выбора тарифа., `test_traffic_gb_for_tariff_falls_back_to_settings_without_override`, `test_total_limit_stays_unlimited_when_base_is_unlimited` — Регрессия: раньше ``база(0) + докупки`` превращало безлимитного триального
...
- `tests/handlers/test_admin_device_limit_syncs_companion.py` — Python-модуль
  Классы: нет
  Функции: `test_device_limit_push_mirrors_onto_the_companion`, `test_squad_and_traffic_pushes_do_not_touch_the_companion` — Only the device-limit screen opts into companion mirroring.
...
- `tests/handlers/test_admin_limited_companion_view.py` — Python-модуль
  Классы: нет
  Функции: `companion_enabled`, `test_block_shows_panel_id_and_companion_traffic`, `test_block_reports_top_ups_on_top_of_the_base_quota`, `test_block_mirrors_main_limit_for_trial_subscriptions` — Триальный компаньон зеркалит лимит самого триала, а не LIMITED_COMPANION_TRAFFIC_GB., `test_block_marks_an_unlimited_companion`, `test_block_is_empty_without_a_companion`, `test_block_is_empty_when_the_feature_is_off`, `test_sync_button_resyncs_the_companion_and_redraws`, `test_sync_button_reports_a_failed_panel_sync`
...
- `tests/handlers/test_admin_tariff_server_limits.py` — Python-модуль
  Классы: нет
  Функции: `test_card_summarizes_limits_and_has_entry`, `test_screen_lists_allowed_squads_with_limits`, `test_screen_lists_all_squads_when_tariff_allows_all`, `test_companion_squad_visible_even_when_not_allowed` — Лимит компаньона независим от allowed_squads — добавлять его туда не нужно, `test_input_sets_limit_without_mutating_stored_dict`, `test_zero_removes_limit`, `test_invalid_input_keeps_state`
...
- `tests/services/test_limited_squad_service.py` — Python-модуль
  Классы: нет
  Функции: `test_disabled_tariff_reports_no_squads_and_zero_base`, `test_enabled_tariff_reports_squads_and_base`, `test_none_tariff_is_disabled`, `test_get_limited_squad_uuids_for_subscription_loads_tariff_by_id`, `test_get_limited_squad_uuids_for_subscription_without_tariff_id`, `test_get_limited_squad_uuids_for_subscription_ignores_the_enabled_flag` — Codex-ревью PR #40 (P1): окно между выключением limited_traffic_enabled, `test_get_raw_limited_squad_uuids_ignores_the_enabled_flag`, `test_get_raw_limited_squad_uuids_none_tariff`, `test_purchase_lives_exactly_30_days_from_its_own_purchase_time`, `test_multiple_purchases_have_independent_expires_at`, `test_remove_purchase_takes_from_oldest_first`, `test_remove_purchase_caps_at_available_amount`, `test_remove_purchase_ignores_expired_rows`, `test_remove_purchase_noop_for_non_positive_amount`, `test_expired_purchase_lowers_effective_limit`, `test_effective_limit_is_zero_unlimited_when_base_is_zero`, `test_main_traffic_never_enters_limited_usage` — MAIN-нода не передаётся в fetch — даже если панель вернула бы по ней данные, их некому суммировать., `test_multiple_limited_nodes_are_summed`, `test_usage_ignores_other_users_on_same_nodes`, `test_multiple_limited_squads_pool_their_nodes`, `test_fetch_returns_none_not_zero_when_panel_call_fails` — Транзиентный сбой панели — это не «трафика не было»: fetch не должен, `test_refresh_preserves_previous_usage_when_panel_call_fails`, `test_squad_removed_when_usage_reaches_base_limit` — п.3: база 50 ГБ, usage 50 ГБ → LIMITED squad снимается., `test_main_squads_untouched_when_limited_removed` — п.4 отдельно: несколько MAIN squads — все остаются после снятия LIMITED., `test_purchase_restores_limited_squad` — п.5: +20 ГБ докупка поднимает effective limit выше usage → LIMITED возвращается., `test_expired_purchase_removes_limited_squad_again` — п.10: usage=85, лимит был 100 (база 50 + докупка 50) — докупка истекла,, `test_patch_is_sent_even_when_computed_state_matches_the_flag` — Reconcile, а не delta: PATCH шлётся каждый цикл, даже когда вычисленное, `test_first_activation_adds_squad_despite_default_active_flag` — Регрессия: даже когда вычисленное ``should_be_active`` для новой подписки, `test_disabled_tariff_never_touches_panel` — Механика выключена на тарифе — sync_limited_squad_state не лезет в панель вообще., `test_deactivate_orphaned_removes_squad_and_clears_flag`, `test_deactivate_orphaned_noop_without_a_tariff`, `test_push_usage_noop_without_merger_settings`, `test_push_usage_noop_without_a_token`, `test_push_usage_noop_when_limit_is_zero` — limit_gb<=0 = безлимит — показывать в списке серверов нечего., `test_push_usage_sends_expected_payload`, `test_push_usage_marks_trial_subscriptions` — Триал определяется по данным бота (is_trial), а не по статусу панели., `test_push_usage_logs_but_does_not_raise_on_rejection`, `test_push_usage_logs_but_does_not_raise_on_network_error`
...
- `tests/services/panel_sync/test_paid_traffic_hold.py` — Python-модуль
  Классы: нет
  Функции: `test_stale_smaller_panel_limit_is_held_right_after_a_purchase` — Подписка #1170: триал(5GB) -> Базовый(0=безлимит), панель ещё не догнала., `test_stale_smaller_finite_panel_limit_is_held_too`, `test_larger_panel_limit_is_still_taken_after_a_purchase` — Расширили лимит ещё и в панели (вручную) — это не откат, берём как раньше., `test_hold_expires_with_the_window_and_without_a_payment`, `test_equal_limit_is_a_noop`
...
- `tests/utils/test_pricing_utils.py` — Python-модуль
  Классы: `TestCalculatePricePerMonth` (4 методов), `TestBuildDynamicValues` (2 методов), `TestTariffPeriodIntrinsicDiscount` (5 методов), `TestTariffPeriodExtrapolatedPrice` (2 методов)
  Функции: нет
